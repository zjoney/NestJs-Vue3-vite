    ...
    async buildRoutesAction(): Promise<AppRouteRecordRaw[]> {
      ...
      const getAllMenuData = () => {
        return getActiveMenu().then((menu) => {
          console.log(menu, 'mm enu');
          return convertMenuTree(menu);
        });
      };
      let backRouteList;
      try {
        backRouteList = await getAllMenuData();
        // backRouteList = JSON.parse(backRouteList);

        backRouteList = wrapperRouteComponent(backRouteList);
        backRouteList = parseRouteRoles(backRouteList);
        backRouteList = addPageNotFoundAtFirst(backRouteList);
      } catch (e) {
        console.error(e);
      }
      backRouteList = asyncRoutes;
